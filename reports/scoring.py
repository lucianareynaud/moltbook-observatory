...
def score(features: WeeklyFeatures, prior_week_features: Optional[WeeklyFeatures] = None) -> WeeklyScores:
    """
    ...
    if health.total_requests == 0:
        ...
        if overall_availability < AVAILABILITY_CRITICAL_THRESHOLD:
            anomalies.append(AnomalyFlag(
                ...
                context={"availability": overall_availability, "threshold": AVAILABILITY_CRITICAL_THRESHOLD},
            ))
            ...
            anomalies.append(AnomalyFlag(
                ...
                context={"availability": overall_availability, "threshold": AVAILABILITY_WARNING_THRESHOLD},
            ))
        ...
        if error_rate > ERROR_RATE_WARNING_THRESHOLD:
            anomalies.append(AnomalyFlag(
                ...
                context={"error_rate": error_rate, "threshold": ERROR_RATE_WARNING_THRESHOLD},
            ))
    ...
    if prior_week_features is not None:
        ...
        if prior_total > 0:
            ...
            if abs_change_ratio > VOLUME_CHANGE_WARNING_THRESHOLD or abs_change_ratio < (1.0 / VOLUME_CHANGE_WARNING_THRESHOLD):
                anomalies.append(AnomalyFlag(
                    severity="warning",
                    category="volume",
                    message=f"Significant volume change: {volume_change_pct:+.1f}% vs prior week",
                    context={"volume_change_pct": volume_change_pct, "current": current_total, "prior": prior_total},
                ))
